...
function secret() {
  return process.env.BETA_SESSION_SECRET || "dev-beta-secret-change-me";
}

function signature() {
  return createHmac("sha256", secret()).update("kudaidem-beta-access-v1").digest("hex");
}
...
export function betaCookieValue() {
  return signature();
}
...
export async function redeemInvite(code: string) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (!invite || !invite.active || invite.uses >= invite.maxUses || (invite.expiresAt && invite.expiresAt < new Date())) {
      return null;
    }
