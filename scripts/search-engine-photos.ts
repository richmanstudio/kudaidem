...
function candidatesFromHtml(html: string): Candidate[] {
  ...
  const fallback = /["']murl["']\s*:\s*["'](https?:\\?\/\\?\/[^"']+)["']/gi;
  for (const match of html.matchAll(fallback)) {
    result.push({ murl: match[1].replace(/\\\//g, "/") });
    if (result.length >= 24) break;
  }
  return result;
}
...
async function validImage(url: string) {
  ...
  try {
    ...
    if (length > 0 && length < 8_000) return false;
    return true;
  } catch {
    return false;
  } finally {
...
async function resolve(place: Place) {
  ...
  if (!html) return null;
  const candidates = candidatesFromHtml(html);

  for (const candidate of candidates.slice(0, 12)) {
    const imageUrl = candidate.murl ? clean(candidate.murl) : "";
    if (!imageUrl || !/^https?:\/\//i.test(imageUrl)) continue;
    if (!(await validImage(imageUrl))) continue;
    const sourceUrl = clean(candidate.purl) || `${SEARCH_ENDPOINT}?${params}`;
...
async function main() {
  ...
  const matches = await concurrent(missing, CONCURRENCY, async (place, index) => {
    ...
    if ((index + 1) % 20 === 0 || found) {
      console.log(`${index + 1}/${missing.length} ${place.name}: ${found ? "found" : "miss"}`);
    }
