...
for (const place of data as Array<Record<string, unknown>>) {
  ...
  for (const key of ["sourceUrl", "imageUrl", "imageSourceUrl"]) {
    let parsed: URL;
    try {
  ...
  const rights = String(place.imageRights);
  if (rights === "APPROVED") approvedPhotos += 1;
  else reviewPhotos += 1;
}
...
console.log(
  ...
  `${approvedPhotos} approved photos, ${reviewPhotos} requiring rights review`,
);
